...
import { auditTrail as fallbackAuditTrail } from '@/lib/mock-data';
...
export default function AuditPage() {
  const [events, setEvents] = useState<any[]>(fallbackAuditTrail);
      ...
      >
        {verified ? (
          <>
            ...
            <span className="text-xs text-muted-foreground">— all {events.length} events have valid integrity</span>
          </>
        ) : (
          <>
            <motion.div
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_400px]">
        ...
        <div className="rounded-xl border border-border bg-card/30 backdrop-blur-sm p-5">
          ...
          <div className="space-y-1">
            {events.map((event, i) => {
              const seq = event.event_id || event.sequence || i + 1;
              const agent = event.agent_id || event.agentId || 'agent';
              const action = event.action || (event.tool ? `${event.tool}.${event.action}` : 'action');
              ...
              return (
                ...
                >
                  ...
                  {event.bedrock && (
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-border/50 bg-muted/30 text-muted-foreground shrink-0">
                      Bedrock {event.bedrock.prompt_attack_detected ? 'Attack' : event.bedrock.sensitive_information_detected ? 'PII' : 'Safe'}
                    </span>
                  )}
